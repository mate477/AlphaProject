...
type ProductController() =
    ...
    //Monitors
        ...
        keyboardProducts @ mouseProducts @ monitorProducts
    ...
    member _.GetById(id: int) : IActionResult =
        ...
        match foundProduct with
        | Some product -> base.Ok(product)
        | None -> base.NotFound()

    [<HttpPost>]
    member _.Create([<FromBody>] product: Product) : IActionResult =
        let updatedList = product :: products
        products <- updatedList
        base.Ok(product)

    [<HttpDelete("{id}")>]
    member _.Delete(id: int) : IActionResult =
        let exists = products |> List.exists (fun p -> p.Id = id)
        if exists then
            let filtered = products |> List.filter (fun p -> p.Id <> id)
            products <- filtered
            base.Ok()
        else
            base.NotFound()
